...
Ray GetScreenToWorldPointWithZDistance(Vector2 position, Camera camera,
    ...
    if (camera.projection == CAMERA_PERSPECTIVE) {
        ...
        matProj = MatrixPerspective(
            camera.fovy * DEG2RAD, ((double)screenWidth / (double)screenHeight),
            0.01f, zDistance);
...
uint32_t uirend_get_screen_width(void) {
    return GetScreenWidth();
}
uint32_t uirend_get_screen_height(void) {
    return GetScreenHeight();
}

int uirend_was_resized(void) {
    return IsWindowResized();
}

float uirend_get_mousewheel(void) {
...
void uirend_render(Clay_RenderCommandArray renderCommands, Font *fonts) {
    for (int j = 0; j < renderCommands.length; j++) {
        ...
        Clay_BoundingBox boundingBox = {
            roundf(renderCommand->boundingBox.x),
        ...
        case CLAY_RENDER_COMMAND_TYPE_BORDER: {
            ...
            if (config->cornerRadius.bottomLeft > 0) {
                DrawRing((Vector2){roundf(boundingBox.x +
                                          config->cornerRadius.bottomLeft),
        ...
        case CLAY_RENDER_COMMAND_TYPE_CUSTOM: {
            ...
            case CUSTOM_LAYOUT_ELEMENT_TYPE_3D_MODEL: {
                ...
                float scaleValue =
                    CLAY__MIN(CLAY__MIN(1, 768 / rootBox.height) *
                ...
                DrawModel(customElement->customData.model.model,
